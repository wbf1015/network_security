...
string readFile(string file_name){
   ifstream f{file_name, std::ios::binary};
   stringstream ss;
   ss << f.rdbuf();
   auto data = ss.str();
   return data;
...
string getMD5(string plain){
   MD5 test;
   return test.encode(plain);
}
...
int main(int argc,char**argv) {
    ...
    for(int i=1;i<argc;i++){
      ...
      if(arg=="-t"){
         cout<<"MD5 result of 'nankai' is ";
         cout<<getMD5("a")<<endl;
         cout<<"MD5 result of test.txt which contain 'nankai' is ";
         cout<<getMD5(readFile("test.txt"))<<endl;
      }
      if(arg=="-c"){
         string path = (argv[++i]);
         cout<<"The MD5 value of file "<<path<<" is "<<getMD5(readFile(path))<<endl;
      }
      if(arg=="-v"){
         string path = (argv[++i]);
         cout<<"Please input the MD5 value of file"<<endl;
         string old_md5;
         cin>>old_md5;
         cout<<"The old MD5 value of file "<<path<<" you have input is "<<old_md5<<endl;
         cout<<"The old MD5 value of file "<<path<<" that has computed is "<<getMD5(readFile(path))<<endl;
         string new_md5= getMD5(readFile(path));
         if(new_md5==old_md5){
      ...
      if(arg=="-f"){
         string path = (argv[++i]);
         string path_ = (argv[++i]);
         cout<<"The old MD5 value of file "<<path<< " in "<<path_<<" is "<<readMD5(path_)<<endl;
         cout<<"The new MD5 value of file "<<path<< " that has computed is "<<getMD5(readFile(path))<<endl;
         if(readMD5(path_)==getMD5(readFile(path))){
            cout<<"OK! The file is integrated"<<endl;
         }else{
            cout<<"Match Error! The file has been modified!"<<endl;
         }
